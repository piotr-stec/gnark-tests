...
describe("Verifier", function () {
  async function deployVerifier() {
    ...
    const Verifier = await hre.ethers.getContractFactory("VerifierFibonacciPublicInputs");
  ...
  describe("Verification", function () {
    it("Should verify proof correctly", async function () {
      ...
      const proofData = JSON.parse(
        await fs.readFile(
          path.resolve(__dirname, "../fibonacci_pub_inputs_data/proof_solidity_fib_pub_inputs.json"),
          "utf-8"
        )
      );
      ...
      const witnessData = JSON.parse(
        await fs.readFile(
          path.resolve(__dirname, "../fibonacci_pub_inputs_data/witness_solidity_fib_pub_inputs.json"),
          "utf-8"
        )
      );
